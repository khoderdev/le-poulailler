...
export default function ItemsList({ category, activeTab, onAddItem, onEditItem, onDeleteItem }: ItemsListProps) {
  return (
    <div className="lg:col-span-3 min-h-0 flex flex-col">
      <div className="bg-white rounded-xl shadow-sm flex-1 flex flex-col min-h-0 max-h-[55vh] sm:max-h-[65vh] lg:max-h-none overflow-hidden -mt-8 sm:mt-0">
        ...
        <div className="divide-y flex-1 overflow-y-auto">
          ...
          {category?.items.length === 0 && <div className="p-8 text-center text-gray-500">No items in this category yet.</div>}
        </div>
      </div>
    </div>
  );
