...
interface MenuLink {
  ...
  title: string;
  subtitle: string;
  icon: typeof HiOutlineShoppingBag | typeof HiOutlineClipboardList;
  bgColor: string;
  shadowColor: string;
...
const Home = () => {
  ...
  const dynamicMenuLinks = useMemo(() => {
    if (menus.length === 0) {
      // Fallback to hardcoded menus if no dynamic menus exist
      return menuLinks;
    }
    ...
    return menus.map(menu => ({
      ...
      title: menu.name,
      subtitle: "",
      icon: menu.slug === "shop" ? HiOutlineShoppingBag : HiOutlineClipboardList,
      bgColor: `bg-[${menu.color}]`,
      shadowColor: `shadow-[${menu.color}]/25`,
      customStyle: { backgroundColor: menu.color }
    }));
  }, [menus]);
  ...
  return (
    <div className="relative h-full flex flex-col bg-slate-50 overflow-hidden">
      ...
      {/* Header */}
      <motion.header initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="py-5 px-6">
        <nav className="flex flex-col items-center gap-3">
          <div className="flex items-center gap-1 bg-white/80 backdrop-blur-sm rounded-full px-2 py-2 shadow-sm border border-gray-100">
            {socialLinks.map(({ href, icon: Icon, label, iconClass }, index) => (
              <a key={index} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="p-2.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-full transition-all duration-200">
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-8 select-none!">
        <div className="w-full max-w-lg">
          {/* Logo Section */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="flex justify-center mb-2! ml-4!">
            <Logo size="xl" />
          </motion.div>
